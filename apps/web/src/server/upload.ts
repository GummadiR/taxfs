...
/** P26 — re-read a stored document whose unconfirmed values were lost. */
export async function rescanDocument(userId: string, ws: string, sourceId: string): Promise<UploadReport> {
  ...
  // Already scrubbed at upload; delete the DB record and run the pipeline
  // fresh over the stored bytes (scrub is idempotent on clean bytes).
  ...
  await runDocPipeline(userId, ws, src.raw_ref.split('/').pop() ?? 'document', mediaType, bytes, report);
  return report;
}
